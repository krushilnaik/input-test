...
const GrowthKPIRenderer = ({
	...
	return (
		<div className="callout border-tertiary-gray bg-secondary-black space-y-2 rounded-2xl border p-4">
			{title && (
				<h4 className="text-primary leading-tight font-semibold wrap-break-word whitespace-normal">
					{title}
				</h4>
			)}
			...
			{subtitle && (
				<div className="text-primary-gray flex items-center gap-1 text-sm">
					{!isNegative && !isZero && <ArrowUpIcon size={16} />}
					{isNegative && <ArrowDownIcon size={16} />}
					<span className="flex-1 wrap-break-word whitespace-normal">
						{subtitle}
					</span>
				</div>
			)}
