...
export const tableComponents: Components = {
    ...
    return (
      <div className="border-tertiary-gray rounded-lg border">
        <div className="border-tertiary-gray flex items-center justify-between border-b px-4 py-2">
          ...
          <TableActions tableRef={tableRef} onViewInCanvas={openModal} />
        </div>
        <div className="markdown-table-wrapper overflow-auto">
          <table ref={tableRef} className="w-full border-separate text-sm" {...props}>
            {children}
          </table>
        </div>
      </div>
  ...
  td: ({ children, ...props }) => (
    <td className="text-primary-gray p-3 align-top whitespace-nowrap first:rounded-l-lg last:rounded-r-lg" {...props}>
      {children}
    </td>
  ),
};
